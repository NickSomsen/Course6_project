...
# fit$table[fit$table$logFC > 4.728096,] # dit gaat met de subset functie, maar hierbij kun je maar op één conditie te gelijk filteren.
library(dplyr) # is als het goed is per default al geïnstalleerd

dplyr::filter(wcfs1$table, (wcfs1$table$logFC > 1 | wcfs1$table$logFC < -1) & wcfs1$table$FDR < 1e-99)
wcfs1$table["lp_0372", ]
dplyr::filter(nc8$table, (nc8$table$logFC > 1 | nc8$table$logFC < -1) & nc8$table$FDR < 1e-99)
nc8$table["lp_0372", ]

# met de filter functie van dplyr kun je wel op meerdere condities te gelijk filteren, met de & of | operators net als in Java. Omdat ik 2 packages geïnstalleerd heb die beide een "filter" functie hebben, moet ik aangegeven van welke package ik de filter functie wil gebruiken

# die filtering moeten we dan denk ik ook per stam doen (hierboven is alleen van NC8), en ook moet er dan nog op p-value gefilterd worden, wat denk ik ook wel in één keer kan
