...
# voor beide stammen worden de unieke genen bepaald, zodat ze later uit de dataframes kunnen worden gehaald
rows_in_wcfs1_that_are_not_in_nc8 = wcfs1[!row.names(wcfs1) %in% row.names(nc8), ]
rows_in_nc8_that_are_not_in_wcfs1 = nc8[!row.names(nc8) %in% row.names(wcfs1), ]

# de unieke genen worden eruit gehaald voor beide stammen
wcfs1 = wcfs1[!(row.names(wcfs1) %in% row.names(rows_in_wcfs1_that_are_not_in_nc8)), ]

nc8 = nc8[!(row.names(nc8) %in% row.names(rows_in_nc8_that_are_not_in_wcfs1)), ]
nrow(nc8)
```
De filtering is voor beide stammen apart gedaan. Na de filtering op logFC, FDR en de uitfiltering van unieke genen van beide stammen, bleven er 233 genen (in beide stammen) over.
...
row.names(combined_wcfs1) = combined_wcfs1[, "Row.names"]
combined_wcfs1 = combined_wcfs1[, 2:15] # de laatste paar kolommen worden eruit gehaald, want die horen er niet te staan
...
row.names(combined_nc8) = combined_nc8[, "Row.names"]
combined_nc8 = combined_nc8[, 2:15] # verwijder laatste kolommen
```

Zodat de data makkelijker te lezen is, worden de twee data frames weggeschreven naar een bestand, die vervolgens bijvoorbeeld in excel ingeladen kan worden.
```{r write results}
write.csv(combined_wcfs1, paste(here(), "wcfs1_results.txt", sep = "/"), row.names = TRUE)

write.csv(combined_nc8, paste(here(), "nc8_results.txt", sep = "/"), row.names = TRUE)
```

## KEGG Mapper
![Volledige pathview Glycolysis](lpl00010.pathview.png)
<br>
```{r keggmapper WCFS1, warning=FALSE, message=FALSE}
library(KEGGREST)
library(pathview)

mypathway <- "lpl00010"

selected_wcfs1 = combined_wcfs1[c("lp_0262", "lp_2100", "lp_2102", "lp_0263", "lp_0576", "lp_2702", "lp_2703", "lp_3658", "lp_3659", "lp_3660"), ]
...
selected_nc8 = combined_nc8[c("lp_3658", "lp_0371"), ]
=======
```
<br>
![Glycolysis marked with WCFS1 genes](lpl00010.png)
<br>
```{r keggmapper NC8, warning=FALSE, message=FALSE}
library(KEGGREST)
library(pathview)
selected_nc8 = nc8[c("lp_3658", "lp_0371"), ]
>>>>>>> ae20caf82d20f693dcec430f15f9daf56b411f58
genes <- row.names(selected_nc8)
logFC <- selected_nc8$logFC
names(logFC) <- genes
pathview(gene.data = logFC, species = "lpl", pathway = mypathway,  gene.idtype = "KEGG")

```
![Glycolysis marked with NC8 genes](lpl00010.png)
<br>
